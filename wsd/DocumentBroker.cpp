        ...
        switch (_docState.activity())
        {
            ...
            case DocumentState::Activity::SaveAs:
            {
                if (_docState.isKitDisconnected())
                {
                    ...
                    {
                        clearSaveResponseTracking(/*rejectResponse=*/true);
                        abandonAgentSaveAttempt("Kit disconnected while saving");
                    }
                    else
                        clearSaveResponseTracking(/*rejectResponse=*/false);
                    endActivity();
                }
                else
...
void DocumentBroker::clearSaveResponseTracking(const bool rejectResponse)
{
    if (const auto session = _saveResponseSession.lock())
    {
        if (rejectResponse)
            session->rejectSaveResponse(_saveResponseRequestId);
        else
            session->cancelSaveResponse(_saveResponseRequestId);
    }
    _saveResponseSession.reset();
    _saveResponseRequestId = 0;
}
                                        ...
                                        const Poco::JSON::Object::Ptr& json)
{
    ...
    const auto expectedResponseSession = _saveResponseSession.lock();
    const bool matchingTrackedResponse =
        responseCorrelation && expectedResponseSession == session &&
        responseCorrelation->requestId == _saveResponseRequestId;
    if (matchingTrackedResponse)
    {
        _saveResponseSession.reset();
        _saveResponseRequestId = 0;
    }
    ...
    if (!matchingTrackedResponse)
    {
        LOG_WRN("Ignoring an uncorrelated save response from session ["
                << session->getId() << "] on docKey [" << _docKey << ']');
        return;
    }
                                 ...
                                 const std::shared_ptr<ClientSession>& uploadSession)
{
    ...
    if (forwardToChild(session, command))
    {
        ...
        _saveResponseSession = session;
        _saveResponseRequestId = saveRequestId;
        if (uploadSession)
        {
                                       ...
                                       std::unique_ptr<WopiStorage::WOPIFileInfo> wopiFileInfo)
{
...
#if !MOBILEAPP
    if (wopiFileInfo && wopiFileInfo->getEnableWebsocketURP() &&
        !session->isAgentRenderRequested())
    {
        session->setWebsocketUrpEnabled(true);
        const bool writable = wopiFileInfo->getUserCanWrite() &&
                              !wopiFileInfo->getUserCanOnlyComment() &&
                              !wopiFileInfo->getUserCanOnlyManageRedlines() &&
                              !session->isReadOnly() &&
                              !CommandControl::LockManager::isLockedReadOnlyUser();
        session->setWritePermission(writable);
        session->setWritable(writable);
        session->setReadOnly(!writable);
        LOG_DBG("Prepared direct URP transport: writable=" << writable);
    ...
    try
    {
        ...
        if (session->isAgentRenderView())
        {
            for (const auto& sessionIt : _sessions)
            {
                if (sessionIt.second->isAgentRenderView())
                {
                    throw std::runtime_error(
                        "An agent render view is already attached to this document");
                }
            }
        }
        // Request a new session from the child kit.
        std::string message = "session " + id + ' ' + _docKey + ' ' + _docId +
                              (session->isAgentRenderView() ? " urp=1" : " urp=0");
...
bool DocumentBroker::detachUrpTransport(const std::shared_ptr<ClientSession>& session)
{
    ASSERT_CORRECT_THREAD();
    LOG_ASSERT_MSG(session, "Got null direct URP transport");

    const auto registeredTransport = _urpTransport.lock();
    if (!registeredTransport || registeredTransport != session)
        return false;

    if (_agentSaveAttempt &&
        _agentSaveAttempt->authoritySessionId() == session->getId())
    {
        clearSaveResponseTracking(/*rejectResponse=*/true);
        abandonAgentSaveAttempt("direct URP transport removed");
        if (_docState.activity() == DocumentState::Activity::Save)
        {
            _saveManager.setLastSaveResult(/*success=*/false, /*newVersion=*/false);
            endActivity();
        }
        broadcastSaveResult(false, "Agent save transport closed");
    }

    _urpTransport.reset();
    _pendingUrpResponse.clear();
    _pendingUrpResponseTime.reset();
    session->bindAgentViewId(-1);
    LOG_DBG("Cleared direct URP transport");
    return true;
}
                                            ...
                                            const std::string& operationId)
{
    ...
    const std::shared_ptr<ClientSession> agentSession = _urpTransport.lock();

    if (!agentSession || !agentSession->getAuthorization().matchesToken(bearerToken))
    {
    ...
    for (const auto& sessionEntry : _sessions)
    {
        ...
        if (session->isAgentRenderView() && session->isLive() && session->isWritable() &&
            session->getKitViewId() == agentSession->getBoundAgentViewId() &&
            session->getAuthorization().matches(agentSession->getAuthorization()) &&
                                    ...
                                    const std::string& message, bool binary)
{
    ...
    if (message.starts_with("urp "))
    {
        ...
        if (registeredTransport && registeredTransport != session)
        {
            LOG_WRN("Rejecting a second direct URP transport for this document");
            return false;
        }

        int resolvedViewId = -1;
        std::size_t matchingViews = 0;
        for (const auto& entry : _sessions)
        {
            const auto& candidate = entry.second;
            if (candidate->isAgentRenderView() && candidate->isLive() &&
                candidate->isWritable() &&
                candidate->getAuthorization().matches(session->getAuthorization()))
            {
                resolvedViewId = candidate->getKitViewId();
                ++matchingViews;
            }
        }

        if (matchingViews != 1 ||
            (session->hasRequestedAgentViewId() &&
             session->getRequestedAgentViewId() != resolvedViewId) ||
            (session->getBoundAgentViewId() >= 0 &&
             session->getBoundAgentViewId() != resolvedViewId))
        {
            LOG_WRN("Rejecting direct URP transport without one live same-token agent view");
            session->sendTextFrame("error: cmd=urp kind=invalidviewid");
            return false;
        }

        const bool newlyRegistered = !registeredTransport;
        if (newlyRegistered)
        {
            session->bindAgentViewId(resolvedViewId);
            _urpTransport = session;
            _urpTransportEverAttached = true;
            LOG_DBG("Validated direct URP transport binding: view=" << resolvedViewId);

            static constexpr auto MaxPendingUrpResponseAge = std::chrono::seconds(5);
            const bool pendingResponseFresh =
                _pendingUrpResponseTime &&
                std::chrono::steady_clock::now() - *_pendingUrpResponseTime <=
                    MaxPendingUrpResponseAge;
            if (!_pendingUrpResponse.empty() && pendingResponseFresh)
            {
                static constexpr std::string_view header = "urp: ";
                std::vector<char> response(header.size() + _pendingUrpResponse.size());
                std::memcpy(response.data(), header.data(), header.size());
                std::memcpy(response.data() + header.size(), _pendingUrpResponse.data(),
                            _pendingUrpResponse.size());
                LOG_DBG("Forwarding pending URP response: payload-length="
                        << _pendingUrpResponse.size());
                session->sendBinaryFrame(response.data(), response.size());
                _pendingUrpResponse.clear();
                _pendingUrpResponseTime.reset();
            }
            else if (!_pendingUrpResponse.empty())
            {
                _pendingUrpResponse.clear();
                _pendingUrpResponseTime.reset();
                LOG_DBG("Discarded expired pending URP response");
            }
        }

        const bool forwarded =
            _childProcess && _childProcess->sendUrpMessage(message, resolvedViewId);
        if (newlyRegistered && forwarded)
            UNITWSD_CALL_INSTANCE(_unitWsd, onDocBrokerAddSession(_docKey, session));
        return forwarded;
    }
...
{
    const auto transport = _urpTransport.lock();
    if (transport && transport->isAgentSaveEligible())
    {
        LOG_DBG("Forwarding URP response: payload-length=" << len);
        static constexpr std::string_view header = "urp: ";
        ...
        transport->sendBinaryFrame(response.data(), responseSize);
        return;
    }

    if (_urpTransportEverAttached)
    {
        LOG_DBG("Dropping URP response after the direct transport detached");
        return;
    }

    static constexpr std::size_t MaxPendingUrpResponseSize = 64 * 1024;
    if (len == 0 || len > MaxPendingUrpResponseSize)
    {
        _pendingUrpResponse.clear();
        _pendingUrpResponseTime.reset();
        LOG_WRN("Rejecting invalid pending URP response length=" << len);
        return;
    }

    if (!_pendingUrpResponse.empty())
    {
        LOG_WRN("Rejecting a second pending URP response");
        return;
    }

    _pendingUrpResponse.assign(data, data + len);
    _pendingUrpResponseTime = std::chrono::steady_clock::now();
    LOG_DBG("Queued pending URP response: payload-length=" << len);
}
